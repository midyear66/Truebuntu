  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-gray-800">Init/Shutdown Scripts</h2>
      ...
      {showCreate && (
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-5 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
            <input type="text" value={form.name} onChange={e => setForm({...form, name: e.target.value})} placeholder="Script name" className="border rounded px-3 py-2 text-sm" required />
            ...
            <select value={form.when_run} onChange={e => setForm({...form, when_run: e.target.value})} className="border rounded px-3 py-2 text-sm">
              <option value="pre">Pre</option>
              <option value="post">Post</option>
            </select>
            <input type="number" value={form.timeout} onChange={e => setForm({...form, timeout: parseInt(e.target.value) || 30})} placeholder="Timeout (seconds)" className="border rounded px-3 py-2 text-sm" />
          </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Timeout</th>
            ...
            {scripts.map(s => (
              <tr key={s.id} className="border-t hover:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  <span className={`text-xs px-2 py-0.5 rounded ${s.type === 'init' ? 'bg-blue-100 text-blue-800' : 'bg-orange-100 text-orange-800'}`}>
                    {s.type}
                  </span>
                </td>
                <td className="px-4 py-3">
                  <span className={`text-xs px-2 py-0.5 rounded ${s.when_run === 'pre' ? 'bg-yellow-100 text-yellow-800' : 'bg-purple-100 text-purple-800'}`}>
                    {s.when_run}
                  </span>
                </td>
                ...
                <td className="px-4 py-3">
                  <span className={`text-xs px-2 py-0.5 rounded ${s.enabled ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600'}`}>
                    {s.enabled ? 'Enabled' : 'Disabled'}
                  </span>
                </td>
                <td className="px-4 py-3 text-right space-x-2">
                  <button onClick={() => runScript(s.id)} className="text-blue-600 hover:text-blue-800 text-xs">Run</button>
                  <button onClick={() => startEdit(s)} className="text-gray-600 hover:text-gray-800 text-xs">Edit</button>
