...
export default function Disks() {
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-gray-800 mb-6">Disk Health</h2>
          ...
          <div className="bg-white rounded-lg shadow">
            {disks.map(d => (
              <div
                ...
                className={`p-4 border-b cursor-pointer hover:bg-gray-50 ${selectedDisk === d.name ? 'bg-blue-50 border-l-4 border-l-blue-500' : ''}`}
              >
                <div className="flex items-center justify-between">
                  <span className="font-medium">{d.name}</span>
                  <span className="text-sm text-gray-500">{d.size}</span>
                </div>
                <div className="text-xs text-gray-400 mt-1">{d.model || 'Unknown model'}</div>
              </div>
            ))}
            {disks.length === 0 && <div className="p-4 text-sm text-gray-400">No disks found</div>}
          </div>
        </div>
        ...
        <div className="lg:col-span-2">
          {smartData && (
            <div className="bg-white rounded-lg shadow p-5">
              ...
              {smartData.error ? (
                <div className="text-sm text-yellow-700 bg-yellow-50 p-3 rounded">{smartData.error}: {smartData.detail || 'SMART may not be supported on this device'}</div>
              ) : smartData.raw ? (
                <pre className="text-xs bg-gray-50 p-3 rounded overflow-x-auto whitespace-pre-wrap">{smartData.raw}</pre>
              ) : (
                <div className="space-y-4">
                  {smartData.model_name && (
                    <div className="grid grid-cols-2 gap-4">
                      <div><span className="text-xs text-gray-500">Model</span><div className="text-sm">{smartData.model_name}</div></div>
                      <div><span className="text-xs text-gray-500">Serial</span><div className="text-sm font-mono">{smartData.serial_number}</div></div>
                      <div><span className="text-xs text-gray-500">Temperature</span><div className="text-sm">{smartData.temperature?.current ?? 'N/A'}°C</div></div>
                      <div><span className="text-xs text-gray-500">Power On Hours</span><div className="text-sm">{smartData.power_on_time?.hours ?? 'N/A'}</div></div>
                    </div>
                  )}
                  {smartData.ata_smart_attributes?.table && (
                    <div>
                      <h4 className="text-sm font-semibold text-gray-700 mb-2">SMART Attributes</h4>
